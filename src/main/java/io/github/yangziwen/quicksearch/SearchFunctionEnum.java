...
public enum SearchFunctionEnum {
    ...
    private static final ConcurrentMap<VarArgsSQLFunction, SearchFunctionEnum> FUNC_MAPPING = new ConcurrentHashMap<VarArgsSQLFunction, SearchFunctionEnum>() {
    ...
    public static AggregationBuilder generateAggsBuilder(FunctionStmt<?> stmt) {
        SearchFunctionEnum func = FUNC_MAPPING.get(stmt.getExpression().getFunc());
        if (func == null) {
            return null;
        }
